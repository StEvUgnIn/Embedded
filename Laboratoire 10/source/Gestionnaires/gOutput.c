...
typedef enum
	{
	kAffichage, kWait1Sec, kWaitOnUser
	} AffStateMachineEnum;
// Variable de la machine d'état de l'affichage
static AffStateMachineEnum sAffState = kWait1Sec;
			...
			if (true == gCompute.aErrTab[sAlarmNb])
				{
					...
					mLcd_Write('N', 10, 1);
					mLcd_Write(' ', 11, 1);
					mRs232_WriteString("Alarme ");
					mRs232_WriteChar(sAlarmNb+0x30);
...
#else
					sAffState = kWaitOnUser;
#endif
				}
				break;
		case kWaitOnUser:
				while (!mRs232_ReadDataFromBuffer(((UInt8*) &sAlarmNb)) ||
					((sAlarmNb < 0 || sAlarmNb > 7) &&
					mRs232_WriteString("\nSaisie incorrecte, veuillez introduire le numéro de l’erreur (0 à 7) : ")));
				break;
			}
	}
